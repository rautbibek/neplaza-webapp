      ...
      <template v-slot:default>
        <thead>
          <tr>
            ...
            <th class="text-left">Fearure Name</th>
            <th class="text-left">Feature Icon</th>
            <th class="text-left">Actions</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item,index) in feature" :key="index">
            <td>{{ index+1 }}</td>
            <td>{{ item.name}}</td>
...
<script>
  export default {
    ...
    methods: {
      getStatus(){
        axios.get(`/admin/feature/${this.scat_id}`)
             .then(response =>{
      ...
      update() {
            if (this.$refs.form.validate()) {
              this.overlay = true;  
                axios.put(`/admin/feature/`+this.status_id,{
                    name: this.name,
                    icon: this.icon,
                    scategory_id: this.scat_id,
                })
